...
- $image\_load = 10$ The inference server needs to pull a 10GB model from Huggingface Hub, which takes 10 seconds. (You could optimize it by using)
...
- $idle\_time_{s} = 30$ The inference server `s1`, `s2` and `s3` remains idle for a period of 30 seconds before it is automatically scaled down.

In this case, we only charge for 990 seconds.

$running\_time_{s1} + running\_time_{s2} + running\_time_{s3} + 3 \cdot idle\_time_{s} = 990$
