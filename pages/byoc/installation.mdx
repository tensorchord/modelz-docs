...
We offer two installation methods, depending on whether you are using kubernetes or vm. Both are very simple and you can deploy in less than 1 minute! they all need agent token, you can get it from ModelZ dashboard.
...
- `--modelzcloud-region=us-central1-a` is the region of your owner VPC.
- `34.134.190.132` is the LoadBalancer IP for ingress nginx.

You can check if the cluster is ready.
```shell
$mdz server list
NAME   PHASE  ALLOCATABLE     CAPACITY       
cegao  Ready  cpu: 2          cpu: 2         
              memory: 5.3GiB  memory: 5.3GiB 
```

If you want to add other VM as node of your cluster, you can refer this [document](https://docs.open.modelz.ai/server/join).
...
```
# VM
$ k3s kubectl label node cegao ai.tensorchord.server-resource=cpu-1c-2g
# Existed kubernetes
$ kubectl label node cegao ai.tensorchord.server-resource=cpu-1c-2g
```
