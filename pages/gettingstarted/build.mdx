...
Before you create the [**Deployment**](../concepts/deployment), you need to first build it and push to a Docker registry. 
...
- [**Mosec**](../mosec): a high-performance and flexible model serving framework for building ML model-enabled backend and microservices.
- [**Gradio**](../gradio): a simple and flexible framework for building ML model-enabled web apps.

Here we take [**Mosec**](../mosec) as an example to show how to build a Docker image for the [**Stable Diffusion**](../templates/stable-diffusion).

Building an inference server based on our inference framework [**Mosec**](../mosec) could be straightforward. You will need to provide three key components:
...
In the `main.py` file, you need to define a class that inherits from `mosec.Worker` and implements the `forward` method. The `forward` method takes a list of inputs and returns a list of outputs with dynamic batching. You could get more details from the [**Mosec**](../mosec) page.
